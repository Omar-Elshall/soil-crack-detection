...
export default function PostFlightPage() {
  ...
  if (!meta || !id) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center gap-2">
        <span className="font-display text-lg font-bold text-ink-muted">Mission not found</span>
  ...
  const chartHistory: Detection[] = detections.map((d, i) => ({
    crack_ratio_pct: d.crack_ratio_pct, fps: 0, timestamp_ms: i,
  }));

  const mapPoints: DetectionPoint[] = detections
    .filter((d) => d.lat !== 0)
    .map((d) => ({ lat: d.lat, lon: d.lon, crack_ratio_pct: d.crack_ratio_pct, timestamp: d.timestamp }));

  const defaultCenter: [number, number] | undefined = mapPoints.length > 0
    ? [mapPoints[0].lat, mapPoints[0].lon] : undefined;
    ...
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      <div className="px-6 pt-5 pb-3 border-b border-parchment-darker shrink-0">
        <button
          ...
          className="flex items-center gap-1 text-xs font-mono text-ink-muted hover:text-terracotta mb-3 transition-colors"
        >
          <ArrowLeft size={12} /> History
        </button>
        <div className="flex items-end justify-between gap-4">
          <div>
            <h1 className="font-display text-2xl font-bold text-ink tracking-tight">{meta.id}</h1>
            <p className="text-xs text-ink-muted mt-1 font-sans">
              {new Date(meta.start_time).toLocaleString()} · {meta.model}
            </p>
          </div>
          <div className="flex items-center gap-2 shrink-0">
            <a href={csvUrl(id)} download className="flex items-center gap-1.5 px-3 py-1.5 rounded border border-parchment-darker bg-surface/80 text-xs font-mono text-ink-soft hover:bg-parchment-dark transition-colors">
              <Download size={11} /> CSV
            </a>
            <a href={geojsonUrl(id)} download className="flex items-center gap-1.5 px-3 py-1.5 rounded border border-parchment-darker bg-surface/80 text-xs font-mono text-ink-soft hover:bg-parchment-dark transition-colors">
              <Map size={11} /> GeoJSON
            </a>
            <a href={pdfUrl(id)} download className="flex items-center gap-1.5 px-3 py-1.5 rounded border border-terracotta/30 bg-terracotta/10 text-xs font-mono text-terracotta hover:bg-terracotta/20 transition-colors">
      ...
      <div className="flex-1 overflow-hidden p-4">
          ...
          <div className="h-full">
            {mapPoints.length > 0 ? (
              ...
              <div className="h-full flex items-center justify-center flex-col gap-2">
                <span className="font-display text-lg font-bold text-ink-muted">No GPS data</span>
                <p className="text-xs text-ink-faint text-center">
                  Flown without a GPS fix.<br />NED coordinates are in the Raw Data tab.
                </p>
              </div>
            )}
          </div>
        ...
        {tab === "analysis" && (
          <div className="max-w-3xl flex flex-col gap-6">
            <div>
              <h3 className="text-xs font-mono uppercase tracking-widest text-ink-muted mb-3">Crack Coverage Over Time</h3>
              <div className="rounded-md border border-parchment-darker bg-surface/70 p-4 h-40">
                <CrackRatioChart history={chartHistory} />
              </div>
            </div>
            <div>
              <h3 className="text-xs font-mono uppercase tracking-widest text-ink-muted mb-3">Top Detections by Coverage</h3>
              <div className="rounded-md border border-parchment-darker bg-surface/70 overflow-hidden">
                <table className="w-full text-xs font-mono">
                  <thead>
                    <tr className="border-b border-parchment-darker bg-parchment-dark">
                      {["Timestamp", "Alt m", "Hdg °", "Coverage %"].map((h) => (
                        <th key={h} className="text-left px-4 py-2 text-ink-faint font-medium">{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {[...detections]
                      ...
                      .map((d, i) => (
                        <tr key={i} className="border-b border-parchment-darker/50">
                          ...
                          <td className="px-4 py-1.5 text-ink-soft">{d.alt_m.toFixed(1)}</td>
                          <td className="px-4 py-1.5 text-ink-soft">{d.heading_deg.toFixed(0)}</td>
                          <td className={`px-4 py-1.5 font-medium ${d.crack_ratio_pct > 10 ? "text-terracotta" : "text-ink"}`}>
        ...
        {tab === "raw" && (
          <div className="h-full overflow-auto">
            <table className="w-full text-[11px] font-mono">
              <thead>
                <tr className="bg-parchment-dark border-b border-parchment-darker sticky top-0">
              ...
              <tbody>
                {detections.map((d, i) => (
                  <tr key={i} className="border-b border-parchment-darker/40 hover:bg-surface/50">
                    ...
                    <td className="px-3 py-1.5 text-ink-faint truncate max-w-[100px]">{d.mask_filename || "—"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
