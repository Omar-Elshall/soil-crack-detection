  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <StatusBadge value={telem.connected} trueLabel="CONNECTED" falseLabel="OFFLINE" variant="gps" />
        <StatusBadge value={telem.armed} trueLabel="ARMED" falseLabel="DISARMED" variant="armed" />
        <StatusBadge value={telem.mode} variant="mode" />
        <StatusBadge value={gpsOk} trueLabel={`GPS FIX ${telem.satellites}sat`} falseLabel="NO GPS" variant="gps" />
      </div>
      ...
      {(telem.lat !== 0 || telem.lon !== 0) && (
        <div className="rounded-md border border-parchment-darker bg-white/40 px-3 py-2 flex gap-4">
          <div>
